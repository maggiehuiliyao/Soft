...
def createScene(rootNode):
    ...
    # def animate_cables(trunk, factor):
    #     for i in range(4):
    #         trunk.node.getChild(f'cableL{i}').getObject('cable').value = factor * 20
    #         trunk.node.getChild(f'cableS{i}').getObject('cable').value = factor * 10
            
    
    # def sequence(target, factor):
    #     animate(animate_cables, {'trunk': trunk}, duration=2, mode='loop')
    #     yield 2.5
    #     animate(animate_cables, {'trunk': trunk}, duration=2, mode='loop', factor=lambda t: abs(math.sin(t*math.pi)))
    #     yield 2.5
    #     animate(animate_cables, {'trunk': trunk}, duration=2, mode='loop', factor=lambda t: 1-t)

    # # AnimationManager.addAnimation(sequence())
    # animate(sequence, {'target': trunk.cableL0}, duration=2, )
    def animate_cables(trunk, factor):
        for i in range(4):
            trunk.node.getChild(f'cableL{i}').getObject('cable').value = factor * 200
            trunk.node.getChild(f'cableS{i}').getObject('cable').value = factor * 100

    def sequence():
        # Animate with different factors for looping and varying displacement
        yield animate(animate_cables, {'trunk': trunk}, duration=2, mode='loop', factor=1.0)
        yield 2.5
        yield animate(animate_cables, {'trunk': trunk}, duration=2, mode='loop', factor=lambda t: abs(math.sin(t * math.pi)))
        yield 2.5
        yield animate(animate_cables, {'trunk': trunk}, duration=2, mode='loop', factor=lambda t: 1 - t)

    # Register animation with the AnimationManager
    AnimationManager.addAnimation(sequence())
    #################################################################
